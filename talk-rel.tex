...
\begin{cases}
\frac{c_p}{\tnow + \tau}  & \text{if } t \ge \tau \quad\text{ (cost rate if failure after $\tau$)}\\
\frac{c_c}{\tnow + t   }  & \text{if } t <   \tau \quad\text{ (cost rate if failure before $\tau$)}
\end{cases} \\
& \gnow(\tau) = \E[g(\tau \mid \Tsysnow)] = \frac{c_p}{\tnow + \tau} \Rsysnow(\tnow + \tau) + c_c \int_0^\tau \frac{1}{\tnow + t} \fsysnow(\tnow + t) \dd t \\
