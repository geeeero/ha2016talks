...
\uncover<1->{\node at (0,0) %
     {\parbox{\textwidth}{\begin{multline*}
       \Rsys\Big( t \mid \big\{\nktzg,\yktzr,\vec{t}^k\big\}{}^{k=1:K} \Big) = P(\Tsys > t \mid \cdots )\\
...
%                                                      $[P(T_k \le t \mid \ptk)]^{m_k-l_k}$ \\ \hspace*{6ex}
                                $\binom{m_k}{l_k} \int_0^1 \big[\ptk\big]^{l_k} \big[1-\ptk\big]^{m_k-l_k} \times$ \\ \hspace*{6ex}
...
\play\ Bounds for $\Rsys\Big(\,t\,\big|\, \Big\{\nktzg,\yktzr,\vec{t}^k\Big\}{}^{k=1:K} \Big)$
over \raisebox{-1.05ex}{$\PktZc$}'s: %\bigcup\limits_{k=1}^K 
\begin{itemize}[<+->]
