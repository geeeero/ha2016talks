...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \draw (-4,0) -- (4,0);
      \draw[very thick,green] (-4,0) -- (0,0);
      \draw[very thick,blue] (0,0) -- (4,0);
      \draw ( 0,0.05) -- ( 0,-0.05) node[anchor=south] {$P(A)$};
      \draw[dashed] ( 0,0) -- (0,-0.5);
      \draw (-2,0.05) -- (-2,-0.05) node[anchor=south] {$p$} node[anchor=north east,xshift=1cm] {buy $A$ for price $p$};
      \draw ( 2,0.05) -- ( 2,-0.05) node[anchor=south] {$q$} node[anchor=north west,xshift=-1cm] {sell $A$ for price $q$};
    \end{tikzpicture}
  \end{center}
